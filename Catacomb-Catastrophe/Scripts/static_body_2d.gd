...
func push_block(dir: Vector2, raycast: RayCast2D):
	...
	if upright:
		...
		if dir.x != 0:
			...
			push_and_rotate(raycast)
	#Connect signal sarkofagen har bevæget sig, for at tjekke collision med slutningen
	# Så behøver vi ikke tjekke det konstant
	block_moved.emit()
...
# It moves and rotates the sarcophagus
# 
